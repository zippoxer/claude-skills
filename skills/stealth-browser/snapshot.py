"""ARIA Snapshot System for Stealth Browser.

Provides accessibility tree snapshots with refs for element interaction.
Based on the dev-browser Playwright implementation, adapted for nodriver.
"""
...
# === JavaScript Snapshot Script ===
# This is a self-contained JavaScript bundle that generates ARIA accessibility trees.
# When injected into a page, it exposes:
# - window.__stealthBrowser_getAISnapshot() - returns YAML accessibility tree
# - window.__stealthBrowser_selectSnapshotRef(ref) - returns element by ref
# - window.__stealthBrowserRefs - object mapping refs to elements

SNAPSHOT_SCRIPT = """
(function() {
  // Skip if already injected
  if (window.__stealthBrowser_getAISnapshot) return;
  ...
  function getAISnapshot() {
    ...
    window.__stealthBrowserRefs = refsObject;
    return renderAriaTree(snapshot);
  }

  function selectSnapshotRef(ref) {
    const refs = window.__stealthBrowserRefs;
  ...
  window.__stealthBrowser_getAISnapshot = getAISnapshot;
  window.__stealthBrowser_selectSnapshotRef = selectSnapshotRef;
})();
"""


async def get_ai_snapshot(tab: uc.Tab) -> str:
    """Inject snapshot script and return YAML accessibility tree string.

    Args:
        tab: nodriver Tab instance

    Returns:
        YAML-formatted accessibility tree with refs like [ref=e1], [ref=e2]
    """
    # First inject the snapshot script (it's idempotent - checks if already injected)
    await tab.evaluate(SNAPSHOT_SCRIPT)

    # Then call the getAISnapshot function
    result = await tab.evaluate("window.__stealthBrowser_getAISnapshot()")
    return result


async def select_snapshot_ref(tab: uc.Tab, ref: str) -> Optional[uc.Element]:
    """Get element by ref from the last snapshot.

    Args:
        tab: nodriver Tab instance
        ref: Reference string like "e1", "e2", etc.

    Returns:
        nodriver Element if found, None otherwise

    Raises:
        Exception: If no snapshot refs exist or ref not found
    """
    # Use JavaScript to mark the element with a temporary attribute so we can find it
    # nodriver's evaluate returns objects in a special format, so we use strings for simplicity

    # First check if refs exist and the ref is valid, then mark the element
    mark_script = f"""
    (function() {{
        const refs = window.__stealthBrowserRefs;
        if (!refs) return "ERROR:No snapshot refs found. Call getAISnapshot first.";
        const element = refs["{ref}"];
        if (!element) return "ERROR:Ref \\"{ref}\\" not found. Available refs: " + Object.keys(refs).slice(0, 20).join(", ");

        // Mark the element with a temporary attribute so we can find it
        const marker = '__stealth_ref_marker_' + Date.now() + '_' + Math.random().toString(36).slice(2);
        element.setAttribute('data-stealth-ref-marker', marker);
        return marker;
    }})()
    """

    result = await tab.evaluate(mark_script)

    # Check for errors (returned as string starting with "ERROR:")
    ...
    # Find the element by the marker attribute
    element = await tab.select(f'[data-stealth-ref-marker="{marker}"]')

    # Clean up the marker
    cleanup_script = f'document.querySelector(\'[data-stealth-ref-marker="{marker}"]\')?.removeAttribute("data-stealth-ref-marker")'
    await tab.evaluate(cleanup_script)
